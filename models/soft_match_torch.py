...
class SoftMatch(nn.Module):
    def __init__(self, config, word_mat, word2idx_dict):
        ...
        self.bert = BertModel('/home/ps/disk_sdb/yyr/codes/NEROtorch/pretrain_models/bert')
        ...
        self.fc_sent2rel = nn.Linear(self.hidden_size, self.config.num_class)
        self.fc_pat2rel = nn.Linear(self.hidden_size, self.config.num_class)
    ...
    def att_match(self, mid, pat, mid_mask, pat_mask, hidden, keep_prob=1.0, is_train=True):
        ...
        pat_v_d = torch.sum(pat_a.unsqueeze(-1).repeat(1, 1, pat_d.shape[-1]) * pat_d, dim=1)
    ...
    def forward(self, sent: torch.IntTensor, mid: torch.IntTensor, rel_label: torch.IntTensor,
        ...
        # sent_d = self.bert(sent, attention_mask=sent_mask)[0]
        # pat_d = self.bert(pat, attention_mask=pat_mask)[0]
        ...
        if self.is_train is True:
            ...
            pat2rel = self.fc_pat2rel(pat_attn2)
